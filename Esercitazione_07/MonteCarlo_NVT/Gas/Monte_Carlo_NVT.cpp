...
//cycle over pairs of particles
  Epot.open("instant_epot.dat",ios::app);
  Press.open("instant_press.dat",ios::app);
...
//update of the histogram of g(r)
     for(int l=igofr; l<igofr+nbins; l++){
       if(dr >= (l-2)*bin_size && dr < (l-1)*bin_size){
         walker[l]+=2;
       }  
     }
...
// contribution to energy and virial
  ...
  Epot << walker[iv]/npart + vtail << endl;
  Press << rho*temp + (walker[iw] + ptail*npart)/vol << endl;
...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Gofr, Gave, Epot, Pres;
    ...
    for(int l=2; l<n_props; l++){
      delta_v=(pow((l-1)*bin_size,3)-pow((l-2)*bin_size,3)); 
