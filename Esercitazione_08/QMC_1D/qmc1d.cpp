...
double variationalWaveFunction_second(double v)
{
	...
	//return 1./pow(sigma,2) *(exp(-pow(v-mu,2)/(2*pow(sigma,2)))*(pow(v-mu,2)/pow(sigma,2)-1) + exp(-pow(v+mu,2)/(2*pow(sigma,2)))*(pow(v+mu,2)/pow(sigma,2) -1));
}
