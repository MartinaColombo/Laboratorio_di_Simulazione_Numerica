...
//Funzione per creare posizioni generiche su una circonferenza con rispettivo
// nome della città [1,2,3,4,5] e inserisco in matrix (x,y,nome) 
 
 void CreatePosition_OnCircumference(vector<vector<double>> &matrix, int length_cromosoma){
   ...
   random_device rnd;
   default_random_engine gen(rnd());
   uniform_real_distribution<> unif_theta(0.,2.*M_PI);   
   
   for(int i=0; i<length_cromosoma; i++){
     theta=unif_theta(gen);
     //cout<<theta<<endl; 
     matrix.push_back({r*cos(theta), r*sin(theta),(i+1)*1.});   //(x,y,allele)         
   }   
 }
