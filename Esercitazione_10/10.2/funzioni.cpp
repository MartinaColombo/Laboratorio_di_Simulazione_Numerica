...
/******************************************************* Gene  *******************************************************/
 ...
 Cromosoma::Cromosoma(const Cromosoma &sample, vector <int> &allele){
   _gene.resize(allele.size());
   _length=allele.size();
   for(int i=0; i<allele.size(); i++){
     for(int j=0; j<allele.size(); j++){
       if(allele[i]==sample.GetGene()[j].GetAllele()){
         _gene[i].SetPosition(sample.GetGene()[j].GetX(), sample.GetGene()[j].GetY());
         _gene[i].SetAllele(sample.GetGene()[j].GetAllele());
         break;
       }
       else{
         continue;
 ...
 vector <int> Cromosoma::GetAllele(){
   vector <int> allele;
   for(int i=0; i<_length; i++){
     allele[i]=_gene[i].GetAllele();
   }
   return allele;
}
...
/**********************************************  NewGeneration  **********************************************/
 ...
 void NewGeneration::SetCromosoma(int position, Cromosoma &crom){
   _crom[position]=crom;
 }
...
//Funzione per riordinare le città in funzione della distanza quadratica
 ...
 void Measure(NewGeneration& new_gen, int rank, int generation, int length_population, int length_cromosoma, int num_generations){

   ofstream out_L, out_L_ave, out_best;
   double distanza=0;

   if(rank==0){
     out_L.open("L2_rank1.dat", ios::app);
     out_L_ave.open("aveL2_rank1.dat", ios::app);
     out_best.open("Best_crom_rank1.dat", ios::app);
   }
   if(rank==1){
     out_L.open("L2_rank2.dat", ios::app);
     out_L_ave.open("aveL2_rank2.dat", ios::app);
     out_best.open("Best_crom_rank2.dat", ios::app);
   }
   if(rank==2){
     out_L.open("L2_rank3.dat", ios::app);
     out_L_ave.open("aveL2_rank3.dat", ios::app);
     out_best.open("Best_crom_rank3.dat", ios::app);
   }
   if(rank==3){
     out_L.open("L2_rank4.dat", ios::app);
     out_L_ave.open("aveL2_rank4.dat", ios::app);
     out_best.open("Best_crom_rank4.dat", ios::app);
   }


   out_L << generation << " " << new_gen.GetCromosoma()[0].GetDistanza() << endl; //Distanza istantanea in fz della generazione
   for (int j=0; j<length_population/2; j++){
     distanza+=new_gen.GetCromosoma()[j].GetDistanza();
   }
   out_L_ave << generation << " " << distanza*1./(1.*length_population/2.) << endl;  //Stampo su file il valor medio della distanza dei cromosomi ad 					           ogni generazione
   distanza=0;
   if(generation==num_generations){
     for(int i=0; i<length_cromosoma; i++){
       out_best << new_gen.GetCromosoma()[0].GetGene()[i].GetX() << " " << new_gen.GetCromosoma()[0].GetGene()[i].GetY() << " " << new_gen.GetCromosoma()[0].GetGene()[i].GetAllele() << endl;
     }
   //Riscrivo la prima città perché il percorso deve tornare al punto di partenza
     out_best << new_gen.GetCromosoma()[0].GetGene()[0].GetX() << " " << new_gen.GetCromosoma()[0].GetGene()[0].GetY() << " " << new_gen.GetCromosoma()[0].GetGene()[0].GetAllele()<<endl;
  }
   out_L.close();
   out_L_ave.close();
   out_best.close();
