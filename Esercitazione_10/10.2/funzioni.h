...
using namespace std;
 ...
 class Cromosoma{
  ...
  public:
    ...
    Cromosoma(const Cromosoma &, vector <int> &);
 ...
 class NewGeneration{
  ...
  void SetCromosoma(int, Cromosoma &);
 ...
 void CreatePosition_InSquare(vector<vector<double>> &, int);
 void ShufflePosition(vector<vector<double>> &);
 void Fitness(vector <Cromosoma> &);
 void Measure(NewGeneration&, int, int, int, int, int);
